...
j_value::j_value(const jc_string_t& i_val, J_Unit i_unit){
	M_type = Value_Types::STRING;
	M_val.str_val = new jc_string_t(i_val);
	M_has_value_status = true;
	M_units = i_unit;
}
...
static void convert_to_same_type(j_value* i_first_val, j_value* i_second_val){
	if(i_first_val->upcastable(i_second_val->type())){
		...
		throw J_Value_Error(L"Cannot convert Value to Same type in convert_to_same_type function");
	}
...
void j_value::binary_value_operation_no_str_or_bool(
	...
	case Value_Types::DOUBLE:
		...
		break;
	case Value_Types::BOOL:
		throw J_Value_Error(L"Bool in wrong binary value_operation function");
	case Value_Types::STRING:
		throw J_Value_Error(L"String in wrong binary value_operation function");
	default:
...
, M_has_value_status(irk_source.M_has_value_status){
	switch(M_type){
	case Value_Types::STRING:
		M_val.str_val = new jc_string_t(irk_source.as_string());
	default:
		;
	}
}
...
j_value& j_value::operator+=(const j_value& irk_val){
	...
	if(Value_Types::STRING == M_type){
		throw J_Value_Error(L"Cannot use \'+\' on strings yet");
	}
...
public:
	...
	void operator()(const Left_t& i_left, const Right_t& i_right, Ret_t* i_destination)const{
		if(!i_right){
			throw J_Value_Error(L"Division By Zero");
		}
		*i_destination = static_cast<Ret_t>(i_left / i_right);
	}
};
...
j_value& j_value::convert_units(J_Unit i_unit){
	...
	if(!is_convertible(i_unit)){
		throw J_Argument_Error(L"Cannot Convert Units");
	}
...
jc_string_t j_value::as_string()const{
	return cast_to<jc_string_t>();
}
...
template<>
jc_string_t j_value::cast_to()const{
	switch(M_type){
	case j_value::Value_Types::LL_INTEGER:
		return to_wstring(M_val.llint_val);
	case j_value::Value_Types::DOUBLE:
		return to_wstring(M_val.dbl_val);
	case j_value::Value_Types::BOOL:
		return M_val.bool_val ? L"true" : L"false";
	case j_value::Value_Types::STRING:
		return *M_val.str_val;
	case j_value::Value_Types::UNDEFINIED:
		throw J_Value_Error(L"Undefined Value type");
	default:
		throw J_Value_Error(L"Could not convert type to string");
	}
}
...
j_llint j_value::as_llint()const{
	...
	case j_value::Value_Types::STRING:{
		...
		wchar_t* str_pos = nullptr;
		return std::wcstoll(M_val.str_val->data(), &str_pos, 0);
	}
	case j_value::Value_Types::UNDEFINIED:
		assert(!"Type Not Identified but declared to have value");
	default:
...
public:
	typedef jc_string_t return_type;

	

	template<typename Val_t>
	return_type operator()(const Val_t& irk_val)const{
		return to_wstring(irk_val);
	}

	template<>
	return_type operator()(const jc_string_t& irk_val)const{
		return irk_val;
	}

	template<>
	return_type operator()(const bool& irk_val)const{
		return irk_val ? L"true" : L"false";
	}
};

jc_string_t j_value::to_str()const{
...
public:
	...
	typename std::enable_if<!std::is_arithmetic<Val_t>::value, return_type>::type  operator()(const Val_t& /*irk_val*/)const{
		throw J_Value_Error(L"Cannot Negate Non Arithmetic type");
	}

	
	j_value operator()(const bool& /*i_val*/)const{
		throw J_Value_Error(L"Cannot Negate Boolean Type");
	}
...
void j_value::convert_to_type(Value_Types i_type){
	if(!value_status()){
		throw J_Value_Error(L"Cannot convert j_value type when j_value has no value");
	}
	...
	case j_value::Value_Types::UNDEFINIED:
	default:
		throw J_Value_Error(L"Could Not Convert j_value type");
	}
...
J_Value_Error::J_Value_Error(const jc_string_t& ik_message):J_Error(ik_message){}
