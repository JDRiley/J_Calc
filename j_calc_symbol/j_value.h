...
public:
	...
	j_value(bool, J_Unit);
	j_value(const jc_string_t&, J_Unit);
	...
	template<>
	jc_string_t as_type<jc_string_t>()const{
		return as_string();
	}

	template<>
	jc_string_t as_type<const jc_string_t&>()const{
		return as_string();
	}
