...
Type_Bool::Type_Bool(const yy::location& irk_loc)
	:Type_Syntax(irk_loc, L"bool", Symbol_Types::BOOL){
...
j_value Type_Bool::convert_value(const j_value& irk_val)const{
	...
	case j_value::Value_Types::STRING:
		throw J_Value_Error(L"Cannot implicitly convert string to double");
	case j_value::Value_Types::UNDEFINIED:
		throw J_Value_Error(L"Cannot implicitly convert undefined to double");
	default:
		assert(0);
		return j_value();
	}
