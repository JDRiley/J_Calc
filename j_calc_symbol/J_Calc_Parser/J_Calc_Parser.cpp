...
using std::wstringstream; using std::stringstream;
...
j_calc_symbol* J_Calc_Parser::parse(const jc_string_t& irk_string){
...
jc_string_t J_Calc_Parser::get_line(int i_line){
...
void yy::J_Calc_Parsing_Unit::error(const location_type& irk_location, const std::string& irk_string){
	std::wcerr << L"\nError Encountered: " << L" at location " << irk_location << L"\n";
	wstringstream o_str;
	o_str << irk_location;
	
	//at some point i should use this message
	(void)irk_string;
